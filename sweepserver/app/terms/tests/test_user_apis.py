...
class TermsAndConditionsAPITestCase(APITestCase):
    ...
    def test_privacy_policy(self):
        ## 1. normal
        response = self.client.get("/api/v1/privacy_policy/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        ## 2. with version (valid)
        response = self.client.get("/api/v1/privacy_policy/?version=2025-04-17")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["content"], "<p>Sample Content</p>")

    def test_privacy_policy_fail(self):
        ## 1. with version (DNE)
        response = self.client.get("/api/v1/privacy_policy/?version=2025-01-01")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data["error"], "존재하지 않는 약관입니다.")

    def test_terms_of_service(self):
        ## 1. normal
        response = self.client.get("/api/v1/terms_of_service/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        ## 2. with version (valid)
        response = self.client.get("/api/v1/terms_of_service/?version=2025-04-17")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["content"], "<p>서비스 이용약관 내용</p>")

    def test_terms_of_service_fail(self):
        ## 1. with version (DNE)
        response = self.client.get("/api/v1/terms_of_service/?version=2025-01-11")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data["error"], "존재하지 않는 약관입니다.")

        ## 2. bad version format
        response = self.client.get("/api/v1/terms_of_service/?version=2025-01")
